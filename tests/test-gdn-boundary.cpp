// Boundary sweep for GDN NaN instability.
//
// Walks a range of n_seq_tokens values and reports per-backend NaN/Inf counts
// at each point, to find the exact transition where the GDN kernel flips from
// clean to unstable. Also prints graph structure stats (tensor count, op count)
// to detect compute-graph-level changes that might explain the flip.
//
// Build:
//   c++ -std=c++17 -I../include -I.. test-gdn-boundary.cpp \
//       ../src/libggml.so.0.16.0 ../src/libggml-base.so.0.16.0 \
//       -pthread -o test-gdn-boundary
//
// Run from build/bin:
//   cd build/bin && ./test-gdn-boundary [-b <backend>] [--seed N] [--range START END STEP]
//
// Examples:
//   ./test-gdn-boundary                          # default sweep 256..512 step 1
//   ./test-gdn-boundary --range 480 520 1        # narrow window around 512
//   ./test-gdn-boundary --range 1 2048 32        # coarse sweep
//   ./test-gdn-boundary -b "Vulkan"              # only Vulkan backend
...
static void print_usage(const char *prog) {
    fprintf(stderr,
        ...
        "Reports NaN/Inf counts per backend per token count to pinpoint the\n"
        "transition boundary. Also prints graph structure stats.\n"
        "\n", prog);
}
...
int main(int argc, char **argv) {
    ...
    printf("%-8s  %-16s  %-16s  %-24s  %s\n",
           "n_tok", "GPU", "CPU", "Graph (tensors/ops)", "Divergence");
    printf("%-8s  %-16s  %-16s  %-24s  %s\n",
           "--------", "----------------", "----------------", "------------------------", "----------");
    ...
    for (int64_t n = range_start; n <= range_end; n += range_step) {
        ...
        char cpu_str[64], gpu_str[64], div_str[32] = "";
